/*
 ...
 Please compile like this:
     ...
     clang++ main.cpp CrabFitsIO.cpp -o CrabFitsImageCrop_linux_x86_64
     clang++ main.cpp CrabFitsIO.cpp -o CrabFitsImageCrop_mac; cp CrabFitsImageCrop_mac ~/Software/ds9/ds9_mac/
 
 Last update:
     
     2014-12-30
     2015-02-01   do not stop when header has no CRPIX1 CRPIX2
     2015-02-02   make sure the output fits is single frame
...
int main(int argc, char **argv)
{
    ...
    **/
    ...
    { 
        ...
        if(cstrNAXIS1!=NULL && cstrNAXIS2!=NULL) {
            ...
            if( iRectI1>=0 && iRectI0<oldImWidth && iRectJ1>=0 && iRectJ0<oldImHeight && \
               ((iRectI0<iRectI1 && iRectJ0<iRectJ1) || (iRectI0==iRectI1 && iRectJ0<iRectJ1) || (iRectI0<iRectI1 && iRectJ0==iRectJ1)) ) {
                ...
                //
                // read image by block <Updated><20141117><DzLIU>
                oldImage = readFitsImageBlock(cstrFilePath,oldLi,oldLj,(newUi-newLi+1),(newUj-newLj+1),extNumber); // <Updated><20141117><DzLIU>
                //
                if(debug>0) {
                ...
                if(cstrCRPIX1!=NULL && cstrCRPIX2!=NULL) {
                    ...
                    std::cout << "CrabFitsImageCrop: Warning! Fits header of extension " << extNumber << " does not contain CRPIX1 and CRPIX2!" << std::endl; // <Modified><20150201><DzLIU> now just give warning rather than give error.
                }
                //
                // also modify BITPIX
                if(debug>0) {
                ...
                if(extNumber>0) {                                       // <TODO> do we need to copy the main header??
                    ...
                    char *newBITPIX = (char *)"-64"; errStatus = modKeyword("BITPIX",newBITPIX,mainHeader);
                                                  ...
                                                  // <TODO>
                                                  // <TODO> still not fully compatible with galfit
                                                  // <TODO> 
        ...
        std::cout << "Usage: \n    CrabFitsImageCrop Arp193_HST_7219_21_NIC_NIC2_F110W_drz.fits -ext 1 -cen 1173 269 200 -out new.fits" << std::endl;
        std::cout << "Usage: \n    CrabFitsImageCrop Arp193_HST_7219_21_NIC_NIC2_F110W_drz.fits -ext 1 -rec 0 0 199 199 -out new.fits" << std::endl;
    }
    return 0;
}
