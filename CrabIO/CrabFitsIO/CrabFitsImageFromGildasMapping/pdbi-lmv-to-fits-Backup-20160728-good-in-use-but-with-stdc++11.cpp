        ...
        if(fp.is_open()) {
            ...
            if(dblCRPIX1<=0 || dblCRPIX2<=0 || dblCRPIX3<=0) {
                ...
                if( (dblPixScaleX<-1e-30||dblPixScaleX>1e-30) && (dblPixScaleY<-1e-30||dblPixScaleY>1e-30) ) { // check non-zero
                    ...
                    strPixScaleX = mesprintf("%g", -dblCDELT1); // <TODO><20160722> the minus sign? GILDAS always gives positive CDELT1?
                    strPixScaleY = mesprintf("%g", dblCDELT2);
                    // Further convert to string type pixel size // <TODO><20160722>
