...
std::vector<std::string> CrabTableReadColumn(const char *InputFile, const char *InputColHead, int *OutputRowNumber = NULL, const char *CommentMark = "#", const char *BreakupMark = " ");
...
std::vector<std::string> CrabTableReadColumn(const char *InputFile, const char *InputColHead, int *OutputRowNumber, const char *CommentMark, const char *BreakupMark)
{
    //
    ...
    if(strlen(InputColHead)>0 && strlen(InputFile)>0) {
        ...
        if (backstory.is_open()) {
            while (backstory.good()) {
                ...
                if(debug) {
                    std::wcout << "CrabStringReadColumn: Debuging: " << "commark = " << commark << std::endl;
                    std::wcout << "CrabStringReadColumn: Debuging: " << "trline = " << trline << std::endl;
                    std::wcout << "CrabStringReadColumn: Debuging: " << "trline.find(commark) = " << trline.find(commark) << std::endl;
                }
                ...
                if(-1==colpos) {
                    if(debug) {
                        ...
                        std::wcout << "CrabStringReadColumn: Debuging: " << "locale = " << locale << std::endl;
                        std::wcout << "CrabStringReadColumn: Debuging: " << "colname = " << colname << std::endl;
                        std::wcout << "CrabStringReadColumn: Debuging: " << "line = " << line << std::endl;
                ...
                if(colpos>=0) {
                    for(int j=0; j<colname.length(); j++) {
                        colvalue = CrabStringFindWholeWord(line,colpos+j,breakup); // <updated><20150615><dzliu>,breakup
                        if(!colvalue.empty()){
