    ...
    if(strlen(InputColHead)>0 && strlen(InputFile)>0) {
        ...
        if (backstory.is_open()) {
            while (backstory.good()) {
                ...
                if(-1==colpos) {
                    colpos = ctrcstrfindwholeword(line,colname);
                ...
                if(colpos>=0) {
                    for(int j=0; j<colname.length(); j++) {
                        colvalue = ctrcstrfindwholeword(line,colpos+j);
                        if(!colvalue.empty()){
                            colvalue = ctrcstrtrim(colvalue);
            ...
            while (backstory.good()) {
                ...
                if(1==1) {
                    ...
                    for(long j=0; j<line.length(); j++) {
                        colvalue = ctrcstrfindwholeword(line,j);
                        if(!colvalue.empty()){
                            // debugTmpArr.push_back(colvalue); // std::cout << "CrabStringReadColumn: Debuging: j="<< j << " colvalue=" << colvalue << std::endl;
                            colpos--;
                    ...
                    if(colpos<0) {
                        ...
                        std::cout << "CrabStringReadColumn: Empty line! <TODO> Stop or not?" << std::endl;
                    } else if(colpos>0) {
                        OutputArray.push_back(colvalue);
                        colcnt++;
                        // std::cout << colvalue << std::endl; // <TODO> Verbose
