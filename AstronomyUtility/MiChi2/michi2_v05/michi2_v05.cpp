...
void *mnchi2parallel(void *params)
{
    ...
    if(debug>=2) {for(int i=0; i<pParams->SDLIBList.size(); i++) {std::cout << process_info.str() << ": Checking pParams->SDLIBList["<<i<<"]->NPAR.size() = " << pParams->SDLIBList[i]->NPAR.size() << std::endl;}}
    if(debug>=2) {for(int i=0; i<pParams->SDLIBList.size(); i++) {std::cout << process_info.str() << ": Checking pParams->SDLIBList["<<i<<"]->CPAR.size() = " << pParams->SDLIBList[i]->CPAR.size() << std::endl;}}
    if(debug>=2) {for(int i=0; i<pParams->SDLIBList.size(); i++) {std::cout << process_info.str() << ": Checking pParams->SDLIBList["<<i<<"]->TPAR.size() = " << pParams->SDLIBList[i]->TPAR.size() << std::endl;}}
    //
    // initialize pParams
