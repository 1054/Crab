...
void *mnchi2parallel(void *params)
{
    ...
    if(debug>=2) {for(int i=0; i<pParams->SDLIBList.size(); i++) {std::cout << process_info.str() << ": Checking pParams->SDLIBList["<<i<<"]->NPAR = " << pParams->SDLIBList[i]->NPAR << std::endl;}}
    if(debug>=2) {for(int i=0; i<pParams->SDLIBList.size(); i++) {std::cout << process_info.str() << ": Checking pParams->SDLIBList["<<i<<"]->CPAR = " << pParams->SDLIBList[i]->CPAR << std::endl;}}
    if(debug>=2) {for(int i=0; i<pParams->SDLIBList.size(); i++) {std::cout << process_info.str() << ": Checking pParams->SDLIBList["<<i<<"]->TPAR = " << pParams->SDLIBList[i]->TPAR << std::endl;}}
    //
    // initialize pParams
