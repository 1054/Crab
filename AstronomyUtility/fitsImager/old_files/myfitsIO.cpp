...
int readFitsHeader(const char *FilePath, int xtension, char **HeaderText, int *HeaderPosition, int *HeaderSize)
{
    ...
    if(fp==NULL)
    {
        printf("readFitsHeader()  Fits File %s can not be opened.\n",FilePath);
        return -5;//file not exist. cannot open it.
    }
...
int readFitsHeaderExtensions(const char *FilePath)
{
    ...
    if(fp==NULL)
    {
        printf("readFitsHeaderExtensions()  Fits File %s can not be opened.\n",FilePath);
        return -1;
    }
    /*Rewind the file pointer and initial parameters.*/
    fseek(fp,0,SEEK_SET);
...
int modKeyword(const char *strKeyName, const char *strKeyValue, const char *strHeader, int *errorcode=0)
{
    int isHeader=0;          //whether this line is simple text (header).
...
int addComment(const char *strComment, const char *strHeader, int *errorcode=0)
{
    int isHeader=0;    //whether this line is simple text (header).
    int flagKeyword=0; //whether this line is a keyword line.
    int flagComment=0; //whether this line is a comment line.
...
int addHistory(const char *strHistory, const char *strHeader, int *errorcode=0)
{
    int isHeader=0;    //whether this line is simple text (header).
    int flagKeyword=0; //whether this line is a keyword line.
    int flagComment=0; //whether this line is a comment line.
...
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
/*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
/*Read FITS Image of float (-32bits) type.*/
double *readFitsImage(const char *chrFilePath)
{
    /*Read fits header.*/
    char   *HeaderData = readFitsHeader(chrFilePath);
    int     HeaderSize = strlen(HeaderData);
    ...
    if(ImageBitpix==32)
    {
        int *data2 = readFitsImageI32(chrFilePath, ImageWidth, ImageHeight, HeaderSize);
    ...
    {
        float *data2 = readFitsImageF(chrFilePath, ImageWidth, ImageHeight, HeaderSize);
        double *data1 = (double *)malloc(ImageSize*sizeof(double));
        for(int i=0; i<ImageSize; i++)
            data1[i] = double(data2[i]);
        return data1;
    ...
    {
        return readFitsImageD(chrFilePath, ImageWidth, ImageHeight, HeaderSize);
    }
    return NULL;
}
...
int writeFitsF(float *data, char *header, char *strFilePath, int *errorcode=0) //TODO:TCHAR?
{
...
int writeFitsD(double *data, char *header, char *strFilePath, int *errorcode=0) //TODO:TCHAR?
{
                                  //errorcode - 61 输入参数无法获取.
                                  //errorcode - 62 输入参数与本函数不匹配.
                                  //errorcode - 50 cannot open file for r/w. unknow reason.
...
int writeFitsFS(float *data, int DataWidth, int DataHeight, char *strFilePath, int *errorcode=0) //TCHAR
{
    int flag=0,m=0;
    FILE *fp;
    const char *header;
...
int writeFitsDS(double *data, int DataWidth, int DataHeight, char *strFilePath, int *errorcode=0) //TCHAR
{
    int flag=0,m=0;
    FILE *fp;
    const char *header;
